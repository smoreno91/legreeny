	...
	public function saveAction() {
		if(Mage::getSingleton('admin/session')->isAllowed('emthemes/megamenupro/save')){
			if ($data = $this->getRequest()->getPost()) {
				parse_str($data['content'],$info);
				$content = $info['menu'];
				if(count($content) > 0 ){
					foreach($content as $k=>$v){
						if($v['type'] == 'link'){
							$content[$k]['label']		=	$this->filterString($v['label']);
							$content[$k]['sublabel']	=	$this->filterString($v['sublabel']);
						}
						if($v['type'] == 'text')
							$content[$k]['text']	=	base64_encode($v['text']);

						if($v['container_css'] != '')
							$content[$k]['container_css']	=	$this->filterString($v['container_css']);
						if($v['css_class'] != '')
							$content[$k]['css_class']	=	$this->filterString($v['css_class']);

					}
					$data['content']	=	Zend_Json::encode($content);
				}else	$data['content'] = "";
				...
				try {
					...
					if ($this->getRequest()->getParam('duplicate')) {
						...
						$model->setIdentifier($data['identifier'].'_duplicate_menuId_'.$this->getRequest()->getParam('id'));
						$model->setId(null);
					}
