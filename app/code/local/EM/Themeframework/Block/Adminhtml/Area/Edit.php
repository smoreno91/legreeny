...
class EM_Themeframework_Block_Adminhtml_Area_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_controller = 'themeframework_area';
		...
		$this->_addButton('save', array(
			'label'     => Mage::helper('themeframework')->__('Save'),
			'onclick'   => 'saveEdit()',
			'class'     => 'save',
		), -100);
        ...
        $this->_formScripts[] = "
			...
			function saveEdit(){
				...
				frm.find('.layout_canvas .block_item,.layout_canvas .grid_item,.layout_canvas .clear').slideDown();
	...
	public function getHeaderText()
	{
		if (Mage::registry('themeframework_area')->getId()) {
			return Mage::helper('themeframework')->__("Edit Layout #%s", $this->escapeHtml(Mage::registry('themeframework_area')->getId()));
		}
		else {
