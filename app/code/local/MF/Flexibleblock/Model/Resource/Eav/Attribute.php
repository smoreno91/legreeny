 ...
 */
class MF_Flexibleblock_Model_Resource_Eav_Attribute extends Mage_Eav_Model_Entity_Attribute
{
    const SCOPE_STORE                           = 0;
    const SCOPE_GLOBAL                          = 1;
    const SCOPE_WEBSITE                         = 2;

    const MODULE_NAME                           = 'MF_Flexibleblock';
    const ENTITY                                = 'flexibleblock_eav_attribute';
     ...
     * @var string
     */
    protected $_eventPrefix                     = 'flexibleblock_entity_attribute';
    /**
     * Event object name
     *
     * @var string
    ...
    {
        $this->_init('flexibleblock/attribute');
    }
    ...
    protected function _beforeSave()
    {
        ...
        if (isset($this->_origData['is_global'])) {
            ...
            if (($this->_data['is_global'] != $this->_origData['is_global'])
                ...
                Mage::throwException(Mage::helper('flexibleblock')->__('Scope must not be changed, because the attribute is used in configurable products.'));
            }
        }
    ...
    protected function _beforeDelete()
    {
        if ($this->_getResource()->isUsedBySuperProducts($this)) {
            Mage::throwException(Mage::helper('flexibleblock')->__('This attribute is used in configurable products.'));
        }
    ...
    public static function initLabels($storeId = null)
    {
        if (is_null(self::$_labels)) {
            ...
            $attributes = Mage::getResourceSingleton('flexibleblock/fblock')->getAttributesByCode();
